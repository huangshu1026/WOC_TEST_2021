...
for(i in 1:round1){
  ...
  write.table(t(c(i, MSE, Optweight0, Optweight1, CWMweight)), "MSEout_inf.csv", sep = ",", append = TRUE, row.names = FALSE, col.names = FALSE)
  ...
  for(j in 1:round2){
    for(l in 1:length(n.vector)){
      ...
      tryCatch({
        ...
        #### output: pvalue ####
        write.table(t(c(i, n, j, mse, pvalue)), file = "res_inf.csv", sep = ",", append = TRUE, row.names = FALSE, col.names = FALSE)
