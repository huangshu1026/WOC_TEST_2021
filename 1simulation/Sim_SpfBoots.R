...
#####################################################
## Prepare the data: inflation rate data
...
for(j in 1:draws){
  for(i in 1:length(n.vector)){
    tryCatch({
      ...
      if(length(weights.retain.slow.index) > 0){
      ...
      write.table(t(c(n, j, pvalue, cv.wa.win, MSE.out, MSE.out.ttest.pvalue)), "InfStates.csv", sep = ",", append = TRUE, row.names = FALSE, col.names = FALSE)
      #write.table(t(c(n, j, pvalue, cv.wa.win, MSE.out, MSE.out.ttest.pvalue)), "UnempStates.csv", sep = ",", append = TRUE, row.names = FALSE, col.names = FALSE)
      print(paste(n, "-", j, sep = ""))  
    }, error = function(e){
      cat("One Error Found!")
    })
